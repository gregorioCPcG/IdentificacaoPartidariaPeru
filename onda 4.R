...
df <- read_csv("df.csv")
df <- df %>%
  filter(S002VS == 4)# onda 

dfao <- read_sav("dfao.sav")
dfao <- dfao %>%
  filter(S002VS == 4)# the same
...
dfao$X047R_WVS
table(df$X047R_WVS)
df$rend_subj <- factor(df$X047R_WVS, levels = c(1, 2, 3), labels = c("Low", "Medium/High", "Medium/High"))
...
df$partidos <- memisc::recode(as.factor(df$E179WVS), "Aprista" <- c(604002),
                               "Socialistas/Progressistas" <- c(604001,604005),
                               "Democracia Cristã/Liberais"<-c(604006,604004),
                               "Fujimoristas"<-c(604008,604014),
                               "Outros" <-c(604003,604007,604009,
                                            604010,604011,604012,604013,-2,-1,2,3))
table(df$partidos)
...
df <- subset(df, select=-c(E235,E069_17,E040))
df[,11:22]
summary(df[,11:22])#ver se tem que remover algo
...
print(df[, 11:22])
summary(df[,11:22])
imp <- mice(df[,11:22], seed=23109)# o nome da base e a seed sempre essa 23109
df[,11:22] <- complete(imp, 1)#sempre a m como  destino da mputação , empre escolher a 1
rm(imp)
summary(df[,11:22])
...
summary(df[,11:22])#verificar



par(mfrow = c(1, 1))
psych::scree(df[,11:22])
nfactors((df[,11:22]))
...
model <- '
  ...
  DesconfiançaInst =~ E069_07 + E069_12 + E069_06
  ManoDuro =~ E117 + E114_invertido + E116_invertido
...
#Optamos por abdicar de usar ManoDuro e ProMercado
...
#dfao$E117#ok
#scores[,3] -> df$manoduro_adptado - nao deu certo
#dfao$E039 # competição é ruim, mas tá invertido , logo é promercado
#scores[,4] -> df$promercado - nao deu certo


par(mfrow = c(1, 2)) # Define uma matriz de plotagem 2x2

hist(df$fundamentalism, breaks = 50, main = "2001 Liberal/Fundamentalismo")
hist(df$desconfiaInst, breaks = 50, main = "2001 Desconfiar de las instituciones")
#hist(df$manoduro_adptado, breaks = 50, main = "2001 Mano Duro Adaptado")
#hist(df$promercado, breaks = 50, main = "2001 Pro Mercado")
...
a <- plot_models(Aprista,SocProg,CristLib,Fuji,auto.label=F,show.values = F, 
                 ...
                 rm.terms = c("autoloc_dir_esqCenter","autoloc_dir_esqRight",
                              "autoloc_dir_esqDK", "interesse"),
                 axis.labels = c("Desconfiar de las instituciones","Fundamentalismo",
...
Apristar2 <- glm(Aprista~Gender+idade_faixa+escolaridade_niveis+rend_subj+
                   autoloc_dir_esq+interesse+
                   fundamentalism
                 ,data = df, family=binomial(link=logit))

SocProgr2<- glm(SocProg~Gender+idade_faixa+escolaridade_niveis+rend_subj+
                  autoloc_dir_esq+interesse+
                  fundamentalism
                ,data = df, family=binomial(link=logit))

CristLibr2 <- glm(CristLib~Gender+idade_faixa+escolaridade_niveis+rend_subj+
                    autoloc_dir_esq+interesse+
                    fundamentalism
                  ,data = df, family=binomial(link=logit))

Fujir2 <- glm(Fuji~Gender+idade_faixa+escolaridade_niveis+rend_subj+
                autoloc_dir_esq+interesse+
                fundamentalism
              ,data = df, family=binomial(link=logit))

tab_model(Apristar2,SocProgr2,CristLibr2,Fujir2,
          show.ci = F, auto.label = F, show.se = T, collapse.se = T, 
          wrap.labels = 60, p.style = "stars",show.p=T,p.threshold = c(0.1,0.05,0.01))
