...
df <- read_csv("df.csv")
df <- df %>%
  filter(S002VS == 3)# onda 

dfao <- read_sav("dfao.sav")
dfao <- dfao %>%
  filter(S002VS == 3)# the same
...
df$partidos <- memisc::recode(as.factor(df$E179WVS), "Aprista" <- c(604002),
                               ...
                               "Democracia Cristã/Liberais"<-c(604006,604017),
                               "Fujimoristas"<-c(604008),
                               "Outros_NONE"<-c(604012,604015,604016,604018,604019,
                                           604020,5,-2,-1))
table(df$partidos)
...
df[,11:24]
summary(df[,11:24])#ver se tem que remover algo
...
print(df[, 11:24])
summary(df[,11:24])
imp <- mice(df[,11:24], seed=23109)# o nome da base e a seed sempre essa 23109
df[,11:24] <- complete(imp, 1)#sempre a m como  destino da mputação , empre escolher a 1
rm(imp)
summary(df[,11:24])
...
summary(df[,11:24])#verificar


psych::scree(df[,11:24])
nfactors((df[,11:24]))
...
hist(df$fundamentalism, breaks = 50, main = "1996 Liberal/Fundamentalismo")
hist(df$desconfiaInst, breaks = 50, main = "1996 Desconfiar de las instituciones")
hist(df$manoduro_adptado, breaks = 50, main = "1996 Mano Duro Adaptado")
hist(df$promercado, breaks = 50, main = "1996 Pro Mercado")
...
df <- subset(df, select=c(Gender,idade_faixa,escolaridade_niveis,rend_subj,
                          autoloc_dir_esq, interesse,
                          Aprista,SocProg,CristLib,Fuji,
                          fundamentalism,desconfiaInst,manoduro_adptado,promercado))
rm(dfao,fit)
...
a <- plot_models(Aprista,SocProg,CristLib,Fuji,auto.label=F,show.values = F, 
            ...
            axis.labels = c("Pro-Mercado","Mano Duro Adaptado",
                            "Desconfiar de las instituciones","Fundamentalismo",
...
Apristar2 <- glm(Aprista~Gender+idade_faixa+escolaridade_niveis+rend_subj+
                 autoloc_dir_esq+interesse+
                 fundamentalism+manoduro_adptado+promercado
               ,data = df, family=binomial(link=logit))
